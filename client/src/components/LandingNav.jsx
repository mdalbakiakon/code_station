...
const LandingNav = () => {
  return (
    <header className="w-[calc(100%-20px)] md:w-[calc(100%-40px)] max-w-7xl h-12 mx-auto bg-(--land-bg-main) rounded-xl p-1 flex justify-between items-center fixed top-5 left-1/2 -translate-x-1/2 z-50 text-(--land-txt-main) leading-none tracking-tighter">
      ...
      <span className="text-lg md:text-xl font-ex relative left-0 md:left-2.5 text-left cursor-pointer">CodeStation</span>

      <nav className="hidden md:flex justify-center items-center font-rg gap-7.5">
        <NavLink
          to="/"
          className={({ isActive }) => (isActive ? "text-(--land-txt-sec) font-ex" : "")}
        >
          Home
        </NavLink>
        <NavLink
      ...
      <span className="absolute inset-0 bg-[conic-gradient(from_-90deg,#f48139,#e57549,#4b11cd,#f48139)] z-10 blur-xs scale-125"></span>
        <span className="relative z-20 text-sm md:text-lg leading-7 tracking-normal text-(--land-txt-main)">Signup</span>
      </button>
